...
//      possibilities.push(String::from(&number[..indexer])+&number[indexer+1..]);
     
//    }  
//    indexer+=1;
// }
// let biggest = &possibilities.iter().map(|x| x.parse::<i32>().unwrap()).min().unwrap();
// println!("{}",format!("BIG HUGE {biggest}"));
// for (x,val) in possibilities.iter().enumerate(){
//      println!("INDEX{x} VALUE {}",&val);
// }
...
// let count = map.entry(cards[card]).or_insert(0);
// *count += 1;
// if(*count == 2){
//         println!("Found minium: {}, Card {}",card,cards[card]);
// }
// }

      // rotate by 90
//       let mut before : [[i32;5];5] = [[0;5];5];

//       for x in 0..before.len(){
//               for y in 0..before[x].len(){
//                      before[x][y] = (x*y) as i32;
//               }
//       }

  
//       let mut rotated : [[i32;5];5] = before.clone();
//       for x in 0..before.len(){
             
//              for y in 0..before.len(){
//               rotated[x][y] = before[before.len()-1-y][x];
//              }
...
//       println!();
//       for x in 0..before.len(){  
//                println!("{:?}",rotated[x].iter().map(|ele| ele.to_string()));
//        }
fn main() {
 //longest sub sequence
 let l = [1,2,3,4,9,1,2,3,4,9];
 
 let mut subsequences : Vec<Vec<i32>> = Vec::new();

      let mut subone: Vec<i32> = Vec::new();
 for x in 0..l.len()-1{
      if (l[x] < l[x+1] ){
            subone.push(l[x].clone());
           
      }else{
            subsequences.push(subone);
            subone = Vec::new()
      }
 }
 
for x in subsequences{
      println!("{}", x.iter().map(|x| x.to_string()).collect::<String>());
}
